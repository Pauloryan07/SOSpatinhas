...
import { registerVet } from "../../services/auth";
...
export default function Card() {
  ...
  const handleRegister = async () => {
    if (!name || !email || !password || !telefone || !crmv) {
      Alert.alert("Aviso", "Preencha todos os campos");
      return;
    }
    setLoading(true);
    try {
      await registerVet({ name, email, password, telefone, crmv });
      router.replace("/home");
    } catch (error: any) {
  ...
  return (
    <View style={styles.card}>
      ...
      <TextInput style={styles.input} placeholder="CRMV" placeholderTextColor="#666" keyboardType="phone-pad" value={crmv} onChangeText={setCrmv} editable={!loading} />
...
const styles = StyleSheet.create({
  card: { flex: 1, width: "100%", backgroundColor: "#FFF", borderTopLeftRadius: 30, borderTopRightRadius: 30, marginTop: 30, padding: 24 },
